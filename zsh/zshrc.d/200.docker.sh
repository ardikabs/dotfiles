...
remote.docker-connect() {
  ...
  ssh_cmd="ssh -fNT -L ${tempdir}/docker.sock:/var/run/docker.sock ${PERSONAL_REMOTE_HOST}"
...
remote.docker-port-forward() {
  ...
  *:*)
    if [[ -n "${DOCKER_HOST}" ]]; then
      ...
      ssh_cmd="ssh -fNT -L 127.0.0.1:${target_port}:127.0.0.1:${destination_port} ${PERSONAL_REMOTE_HOST}"
      if eval "${ssh_cmd}"; then
